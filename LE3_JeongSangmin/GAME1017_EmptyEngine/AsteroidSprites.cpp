	...
	m_angle(270), m_velMax(6.0), m_rotSpeed(5.0), m_thrust(0.5), m_radius(38.0) // Add movement initializers.
{
	...
	m_center = { (m_dst.x + m_dst.w / 2.0f), (m_dst.y + m_dst.h / 2.0f) };
	SetAnimation(1, 0, 1); // Setting idle animation
}
...
void ShipAsteroids::Update()
{
	...
	// Spawn bullet.
	if (EVMA::MousePressed(1)) // MousePressed
	{
		m_bullets.push_back(new Bullet({ 500,0,18,24 }, 
			{m_center.x - 9, m_center.y - 12,18,24}, "sprites", m_angle));
		m_bullets.shrink_to_fit();
		SOMA::PlaySound("fire");
	}
	...
	case STATE_MOVING:
		...
		// Clamp velocities. min(max())
			m_velX = max(m_velX, -(fabs(m_dx) * m_velMax));
			m_velY = max(m_velY, -(fabs(m_dy) * m_velMax));
