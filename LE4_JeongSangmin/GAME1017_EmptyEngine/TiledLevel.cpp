...
TiledLevel::TiledLevel(const unsigned short r, const unsigned short c, const int w, const int h, const char* tileData,
	const char* levelData, const char* tileKey) : m_rows(r), m_cols(c), m_tileKey(tileKey)
{
	ifstream inFile(tileData); // Create the 16 prototypes Tiles
	if (inFile.is_open())
	{
		char key;
		int x, y;
	...
	{
		char key;
		m_level.resize(m_rows); // Set size of the first dimension. We can only use [] after a size is set
		for (unsigned short row = 0; row < m_rows; row++)
		{
			m_level[row].resize(m_cols); // Create 2D vector
			for (unsigned short col = 0; col < m_cols; col++)
			{
				inFile >> key;
				m_level[row][col] = m_tiles[key]->Clone();	// Common prototype method
															// We can use now because vector is set with 2D vector
...
{
	// Clear the tiled level of clones. This also would account for those in m_obstacles
	for (unsigned short row = 0; row < m_rows; row++)
	{
		for (unsigned short col = 0; col < m_cols; col++)
		{
